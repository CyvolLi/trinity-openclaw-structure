...
5. 写入 SQLite：`INSERT INTO photos VALUES (...)`
6. 返回结果文本前加 `MEDIA:album/images/{sender_id}/{ID}.{ext}` 指令
7. 返回保存结果给 Father
...
4. 返回结果时在文本前加 `MEDIA:album/images/{sender_id}/{filename}` 指令
...
Son 返回时，在文本结果前面加上 `MEDIA:album/images/{sender_id}/{文件名}` 指令，Father 转发时 QQ 会自动发送图片到聊天中。
